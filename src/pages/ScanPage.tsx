import { Button, Card, DotLoading, Input, Space, Toast } from "antd-mobile";
...
export function ScanPage() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        if (!disposed) {
          setError(err instanceof Error ? err.message : "店铺不存在");
  ...
  const applyPayload = useCallback((payload: ScanPayload) => {
    ...
    if (payload.mode === "DINE_IN") {
      ...
      if (!validTableSet.has(payload.tableNo)) {
        setError(`桌号 ${payload.tableNo} 不在门店配置中`);
        setScanState("SCAN_FALLBACK");
        return;
      }
    }
  ...
  const decodeWithCamera = async () => {
    if (!("BarcodeDetector" in window)) {
      setError("当前浏览器不支持摄像头扫码，已切换手动录入");
      setScanState("SCAN_FALLBACK");
      return;
    }
    ...
    <div className="mobile-page page-scan">
      ...
      <Card title="扫码入口">
        <Space direction="vertical" block>
          ...
          {scanState === "SCAN_ACTIVE" && <p className="muted">对准二维码后将自动跳转</p>}
        </Space>
      </Card>
